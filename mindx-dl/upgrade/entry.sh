...
function printVersion(){
	echo $(kubectl describe pod -n ${1} $(kubectl get pods -A | grep ${2} | awk '{print $2}' | head -n 1) | grep Image: | awk '{print $2}')
}
...
function upgrade(){
  	...
  	# Pause
  	read -p "Do you want to continue upgrade?(yes/no)" continue

  	while [ "$continue" != 'yes' ] && [ "$continue" != 'no' ];do
        	read -p "Invalid input. Do you want to continue upgrade?(yes/no)" continue
  	done
  	...
  	if [ "$(grep -c "failed=1" ./check_log.txt)" -eq "1" ];then
    		...
    		read -p "Do you want to roll back to previous version?(yes/no)" rollback

    		while [ "$rollback" != 'yes' ] && [ "$rollback" != 'no' ];do
            		read -p "Invalid input. Do you want to roll back to previous version?(yes/no)" rollback
    		done
    		...
    		# Choose whether remove previous version images.
          	read -p "Do you want to remove previous version images?(yes/no)" remove
          	while [ "$remove" != 'yes' ] && [ "$remove" != 'no' ];do
                	read -p "Invalid input. Do you want to remove previous version images?(yes/no)" remove
          	done
