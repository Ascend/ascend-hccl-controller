...
// NewK8SJobWorker ： to create Job Worker
func NewK8SJobWorker(agent *BusinessAgent, job K8sJobInfo, ranktable v1.RankTabler,
	replicasTotal int32) *K8SJobWorker {
	return &K8SJobWorker{
...
func (w *K8SJobWorker) doWork(pod *apiCoreV1.Pod, podInfo *podIdentifier) (forgetQueue, retry bool) {
	// scenario check A: For an identical job, create it immediately after deletion
	// check basis: job uid + creationTimestamp
	if pod.CreationTimestamp.Before(&w.JobCreationTimestamp) {
		// old pod + new worker
...
func (w *K8SJobWorker) Statistic(stopTime time.Duration) {
	for {
		select {
		...
		default:
			if w.taskReplicasTotal == w.cachedPodNum {
				hwlog.Infof("rank table build progress for %s/%s is completed",
					w.JobNamespace, w.JobName)
				w.CloseStatistic()
				return
			}
			hwlog.Infof("rank table build progress for %s/%s: pods need to be cached = %d,"+
				"pods already cached = %d", w.JobNamespace, w.JobName, w.taskReplicasTotal, w.cachedPodNum)
			time.Sleep(stopTime)
		}
	}
}
