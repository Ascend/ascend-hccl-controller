...
func Test_businessWorker_SyncHandler(t *testing.T) {
	...
	tests := []testCaseForWorker{
		newTestCaseForWorker("test1: task ==0,return directly", false, nil, false, nil),
		newTestCaseForWorker("test2: dryrun case,return directly", false, nil, false, mockPodIdentify("delete")),
		newTestCaseForWorker("test3:undefined situation, return directly ", false, nil, false, mockPodIdentify("add")),
		newTestCaseForWorker("test4:add pod and no error returned", false, pod, true, mockPodIdentify("add")),
		newTestCaseForWorker("test5:delete pod and no error returned", false, pod, true, mockPodIdentify("delete")),
	}
...
func newTestCaseForWorker(name string, wantErr bool, pod *apiCoreV1.Pod, podExist bool, podIdentifier *podIdentifier) testCaseForWorker {
...
func Test_businessWorker_handleAddUpdateEvent(t *testing.T) {
	...
	pods = append(pods, pod.DeepCopy())
	tests := []testCaseForWorker{
		newTestCaseForWorker("test1:no device info,run cachezeroPodInfo,no error", false, pods[0], true, mockPodIdentify("add")),
		newTestCaseForWorker("test2: already cache pod info, do nothing", false, pods[1], true, mockPodIdentify("add")),
		newTestCaseForWorker("test3: with device info,run cachePodInfo,no error", false, pods[2], true, mockPodIdentify("add")),
		newTestCaseForWorker("test4: json format error", true, pods[3], true, mockPodIdentify("add")),
	}
	b := newBusinessWorker(mockK8s, nil, nil, false, job)
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
