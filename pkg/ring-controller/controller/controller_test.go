...
func TestController_syncHandler(t *testing.T) {
	...
	tests := []controllerTestCase{
		...
		mackTestCase("test4: delete event but indexer have pod ,should return error", "vcjob/testpod/delete", true),
		mackTestCase("test5: Unfinded situation, should return error", "vcjob/testpod/new", true),
		mackTestCase("test6: Indexer return error, should return error", "vcjob/testpod/delete", true),
		mackTestCase("test7: delete event,no err returned", "vcjob/testpod/delete", false),
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &Controller{
...
func TestNewController(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewController(mockK8s, nil, false, false, 1, 10, 60, mockInformer, make(chan struct{})); !reflect.DeepEqual(got.businessAgent, tt.want.businessAgent) {
